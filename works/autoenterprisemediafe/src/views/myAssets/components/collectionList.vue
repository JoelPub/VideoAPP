    ...
    <div class="panel-poster">
      <div v-if="componentDataList.length" class="poster-content">
        <card
          ...
          :ifBtnFavorite="false"
          :ifCreateEmptyTemplate="1"
          :ifPublicAttr="1"
          :ifPersonalPage="true"
          @downAttr="downAttr({ src: item.sourceUrl || item.coverUrl, fileName: item.fileName, type: item.typeName })"
          @deleteAttr="deleteAttr(item.collectionId, i)"
        >
          <template v-slot:attr>
            <video-attr
              @playVideo="$emit('playVideo', { ...item, isHomemade: 1, ifPublicAttr: 1 })"
              v-if="item.typeName == '视频'"
              :source="{ src: item.sourceUrl || item.coverUrl }"
            ></video-attr>
            <img
              @click="$emit('playVideo', { ...item, isHomemade: 1, ifPublicAttr: 1 })"
              v-else
...
export default {
  ...
  watch: {
    typeNameLabel: {
      handler() {
        this.refreshComponentData('historyCollectList')
      }
    }
  },
  data() {
  ...
  methods: {
    ...
    templateTimeSort() {
      ...
      if (!this.sort) this.refreshComponentData('historyCollectList')
    ...
    refreshComponentData(apiName, fileName) {
      ...
      this.$http[apiName]({
        ...
        if (res.status === 200) {
          this.total = res.data.total
          if (!res.data.list) replaceListItem(this.componentDataList, res.data.historyCollect.list)
