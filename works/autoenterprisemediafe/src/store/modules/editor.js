...
const actions = {
  ...
  updateBackground({commit}, color) {
	 ...
	 * 添加元素
	 */
	addElement({commit, dispatch}, elData){
	  dispatch('addElementWithoutCache', elData)
		commit('addHistoryCache')
	},
  /**
   * 添加元素，但不计入历史记录
   */
  addElementWithoutCache({commit}, elData) {
	...
	elementCommand({commit, dispatch, state}, command){
		...
		switch (command) {
			...
			case 'fontA-':
				...
				break;
			case 'fontB':
				dispatch('resetElementCommonStyle', {fontWeight: +elData.commonStyle.fontWeight >= 700 ? 400 : 700})
				break;
			case 'layerUp':
			...
			case 'layerBottom':
				...
				break;
      case 'imageCrop':
        if (elData.elName !== 'qk-image') {
          return
        }
        commit('updateImageCorpDialogVisible', true)
...
const mutations = {
  updateImageCorpDialogVisible(state, visible) {
    state.isShowImageCorpDialog = visible
  },
  // 更新海报背景色
  updateBackground(state, color) {
    let _index = 0
    state.projectData.pages.forEach((v, index) => {
      if (v.uuid === state.activePageUUID) _index =  index
    })
    /*active.backgroundColor = color*/
    state.projectData.pages[_index].commonStyle.backgroundColor = color
  },
	setPrjectData(state, data){
		state.projectData = data;
	},
  setCanvasSize(state, {width, height}) {
    state.projectData.width = width
    state.projectData.height = height
  },
